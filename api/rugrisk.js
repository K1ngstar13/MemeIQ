export default async function handler(req, res) {
  ...
  if (!summary || typeof summary !== "string") return res.status(400).json({ ok: false, error: "Invalid summary" });
  ...
  try {
    const out = await hfJsonCall(
      ...
      {
        inputs: summary,
        parameters: { candidate_labels: ["rug pull", "legitimate", "high risk", "safe"] },
      },
      { cacheSeconds: 120 }
    );
    ...
    return res.status(200).json({ ok: false, error: e.message || "rugrisk failed" });
  }
}
